...
import con.library.components.books.services.IBookService;
...
public class BookView extends View implements ListView<Book>{
    ...
    private long  inputId(InputOption option) {
        ...
        do {
            ...
            isRetry = isRetry(option, isRetry, exist);
        } while (isRetry);
        return id;
    }

    public static boolean isRetry(InputOption option, boolean isRetry, boolean exist) {
        switch (option) {
            case ADD -> {
                if (exist) {
